...
{
    TicToc tic_toc;
    // parameters[0]是第一个参数，即para_Pose[imu_i], 7维，第i帧位姿
    Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
    Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);

    //parameters[1]是第二个参数，即para_Pose[imu_j], 7维，第j帧位姿
    ...
    //parameters[3]是第四个参数，即para_Feature[feature_index], 1维，第i帧相机坐标系下的逆深度λ
...
#else
    double dep_j = pts_camera_j.z(); // 第i帧到第j帧相机重投影深度
    // 归一化平面残差
    residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>(); // pts_j是第i帧相机归一化平面坐标
#endif
    ...
    if (jacobians)
    {
        ...
        //reduce 表示残差residual对fci（pts_camera_j）的导数（根据不同的相机模型）
