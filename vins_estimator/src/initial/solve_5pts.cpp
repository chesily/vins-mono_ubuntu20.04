...
/**
 * @brief   根据两帧图像特征点匹配关系通过求解本质矩阵得到R,t（尺度未定）
 * @param[in]   corres      对应特征点对
 * @param[out]  Rotation    当前帧（最新帧）到参考帧（第l帧）的旋转矩阵
 * @param[out]  Translation 当前帧（最新帧）到参考帧（第l帧）的平移向量
...
bool MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    if (corres.size() >= 15)
    {
        ...
        /**
         *  Mat cv::findFundamentalMat(  返回通过RANSAC算法求解两幅图像之间的基础矩阵
